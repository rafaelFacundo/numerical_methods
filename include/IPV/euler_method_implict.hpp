...
class EulerMethodImplict : public IPV
{
public:
    EulerMethodImplict(double so, double deltat, std::function<double(double)> function, int numbeofstates);
    ...
    double newtonRaphsonMethod(double Xinitial, double tolerance, std::function<double(double)> function);
};